...
## 2. 通し(3経路)— これは公開基盤の**事前検証**であり、台本本番ではない

**当日の台本本番は DG の画面**(DEMO-00 §7 の手順3・5)で走る。`chain_demo.sh` /
`scene5_evidence.sh` は**公開基盤(および RCOS)の検証用スクリプト**で、台本には出てこない。
本節は「公開基盤側が当日までに 3 経路が通ることを自分で確かめる」ための手順であり、
稽古の場で走らせる実演ではない。台本の registered 取得は DG が **`201` 応答の
`wallet_credential_id` をそのまま使う**流れ(§11.2.2)で、これは下表の `chain_demo.sh` が
検証している流れと同じもの — スクリプトを稽古で回すという意味ではない。
